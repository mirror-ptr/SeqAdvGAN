...
def visualize_perturbation_norms(writer: SummaryWriter, delta: torch.Tensor, step: int):
    ...
    with torch.no_grad():
        # Assuming delta is (B, C, N, W, H) as per Generator output
        if delta.ndim != 5:
        ...
        # Permute delta (B, C, N, W, H) to (B, C, N, H, W) for norm calculations
        delta_for_norm = delta.permute(0, 1, 2, 4, 3)

        linf = linf_norm(delta_for_norm).mean().item() # linf_norm returns (B,), take mean
        l2 = l2_norm(delta_for_norm).mean().item() # l2_norm returns (B,), take mean
...
def visualize_samples_and_outputs(writer: SummaryWriter,
                                  original_image: torch.Tensor,   # 新增：原始图像 (B, C, T, H, W)
                                  feature_delta: torch.Tensor,    # 调整：特征扰动 (B, 128, N_feat, W_feat, H_feat)
                                  adversarial_features: torch.Tensor, # 保持：对抗特征 (B, 128, N_feat, W_feat, H_feat)
                                  original_decision_map: torch.Tensor, # 保持：原始决策图 (B, W, H) 或 (B, H, W)
                                  adversarial_decision_map: torch.Tensor, # 保持：对抗决策图 (B, W, H) 或 (B, H, W)
                                  step: int,
                                  num_samples=4,
                                  sequence_step_to_vis=0,
                                  visualize_decision_diff=True):
    """
    可视化原始图像、特征扰动、对抗特征、原始决策图和对抗决策图。
    将图像记录到 TensorBoard。
    Args:
        writer (SummaryWriter): TensorBoard SummaryWriter 对象。
        original_image (torch.Tensor): 原始图像张量 (B, C, T, H, W)。
        feature_delta (torch.Tensor): 生成的特征扰动 (B, 128, N_feat, W_feat, H_feat) 或类似。
        adversarial_features (torch.Tensor): 对抗性特征层 (B, 128, N_feat, W_feat, H_feat) 或类似。
    ...
    """
    # Ensure necessary tensors are not None or empty
    if original_image is None or original_image.numel() == 0:
        print("Warning: Original image is None or empty. Skipping sample visualization.")
        return

    num_samples_to_show = min(num_samples, original_image.shape[0])
    if num_samples_to_show == 0: return

    # 将张量移动到 CPU (如果不在 CPU 上) 并选取要显示的样本
    original_image_cpu = original_image[:num_samples_to_show].cpu().detach() # Ensure detached
    feature_delta_cpu = feature_delta[:num_samples_to_show].cpu().detach() if feature_delta is not None and feature_delta.numel() > 0 else torch.empty(0)
    adversarial_features_cpu = adversarial_features[:num_samples_to_show].cpu().detach() if adversarial_features is not None and adversarial_features.numel() > 0 else torch.empty(0)


    # --- Visualize Original Image ---
    # original_image_cpu shape: (B, C, T, H, W)
    # We need to visualize one time step from the sequence.
    if original_image_cpu.ndim == 5:
        B_img, C_img, T_img, H_img, W_img = original_image_cpu.shape
        if T_img == 0:
             print("Warning: Original image sequence dimension T is 0. Cannot visualize image.")
        else:
            ...
            if C_img == 1:
                img_to_vis = img_to_vis.repeat(1, 3, 1, 1)
                 ...
                 print(f"Warning: Original image has {C_img} channels. Taking first 3 for visualization.")
                 img_to_vis = img_to_vis[:, :3, :, :]

            # Ensure image is in [0, 1] range (assuming it was normalized)
            # make_grid with normalize=True can handle this if needed, but better to ensure source data is normalized
            # If image data is uint8 [0, 255], convert to float [0, 1] before passing
            # Assuming data loader returns float [0, 1]
        ...
        print(f"Warning: Original image tensor is not 5D ({original_image_cpu.ndim}D). Cannot visualize image.")


    # --- Visualize Feature Perturbation (Delta) ---
    # feature_delta_cpu shape: (B, 128, N_feat, W_feat, H_feat) or similar
    # Use feature_to_visualizable for this
    if feature_delta_cpu.numel() > 0:
        vis_delta = feature_to_visualizable(feature_delta_cpu, sequence_step=sequence_step_to_vis) # (B, C', H', W')
        if vis_delta.numel() > 0: # Check again if feature_to_visualizable returned non-empty
             if vis_delta.ndim == 3: # Should be (B, C', H', W')
                vis_delta = vis_delta.unsqueeze(1) # Ensure 4D for make_grid if it somehow became 3D
                grid_delta = make_grid(vis_delta, nrow=num_samples_to_show, normalize=True)
                writer.add_image(f'Samples/Feature_Perturbation_Delta_Seq{sequence_step_to_vis}', grid_delta, step)
        else:
              print(f"Warning: feature_to_visualizable returned empty tensor for feature delta.")


    # --- Visualize Adversarial Features ---
    # adversarial_features_cpu shape: (B, 128, N_feat, W_feat, H_feat) or similar
    # Use feature_to_visualizable for this
    if adversarial_features_cpu.numel() > 0:
        vis_adv_feat = feature_to_visualizable(adversarial_features_cpu, sequence_step=sequence_step_to_vis) # (B, C', H', W')
        if vis_adv_feat.numel() > 0: # Check again if feature_to_visualizable returned non-empty
             if vis_adv_feat.ndim == 3: # Should be (B, C', H', W')
                vis_adv_feat = vis_adv_feat.unsqueeze(1) # Ensure 4D for make_grid if it somehow became 3D
              ...
              print(f"Warning: feature_to_visualizable returned empty tensor for adversarial features.")


    # --- Visualize ATN Decision Maps (only in Stage 2) ---
    # This part remains largely the same, but should only run if decision maps are provided (e.g., in Stage 2)
    # In Stage 1, original_decision_map and adversarial_decision_map will be None.
    if original_decision_map is not None and adversarial_decision_map is not None:
        ...
        original_decision_map_cpu = original_decision_map[:num_samples_to_show].cpu().detach()
        adversarial_decision_map_cpu = adversarial_decision_map[:num_samples_to_show].cpu().detach()

        # 将决策图转换为适合可视化的格式 (B, 1, H, W) 或 (B, 1, W, H)
        # Need to handle potential (B, W, H) vs (B, H, W) ambiguity in decision map shape
        # Assuming (B, H, W) for now based on decision_layer output in mock ATN
        ...
        # Handle case where only original decision map is provided (less likely in this setup but for completeness)
        elif original_decision_map is not None:
             original_decision_map_cpu = original_decision_map[:num_samples_to_show].cpu().detach()
        ...
        # Handle case where only adversarial decision map is provided (less likely)
    elif adversarial_decision_map is not None:
            adversarial_decision_map_cpu = adversarial_decision_map[:num_samples_to_show].cpu().detach()
...
# visualize_attention_maps 函数保持不变，因为它只处理注意力矩阵，并且已经有 Stage 2 的检查。
def visualize_attention_maps(writer: SummaryWriter,
    ...
    if B == 0 or head == 0 or N1 == 0 or N2 == 0:
         print("Warning: Attention matrix dimensions are zero after checking availability. Skipping attention map visualization.")
         return
